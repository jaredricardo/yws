...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const ywsModalOpen = document.querySelectorAll('.yws-modal__open')
  const ywsModalClose = document.querySelectorAll('.yws-modal__close')

  // Open Modal
  ywsModalOpen.forEach((el) => {
    el.addEventListener('click', () => {
      // lock body
      document.body.classList.add('overflow-hidden')
      // if nearest yws is a video section, make sure to pause initial video on modal open and play full video
      if(el.closest('.yws-section').classList.contains('yws-video-section')) {
        el.closest('.yws-section').querySelector('.initial-video video').pause()
        const iframe = el.closest('.yws-section').querySelector('.yws-modal__inner iframe')
        const srcString = iframe.getAttribute('src')
        iframe.setAttribute('src', `${srcString}&autoplay=1`)
      }
      el.closest('.yws-section').querySelector('.yws-modal__main-area').classList.add('active')
    })
  })

  // Close modal
  ywsModalClose.forEach((el) => {
    el.addEventListener('click', () => {
      // unlock body
      document.body.classList.remove('overflow-hidden')
       // if nearest yws is a video section, make sure to play initial video on modal close and pause full video
      if(el.closest('.yws-section').classList.contains('yws-video-section')) {
        el.closest('.yws-section').querySelector('.initial-video video').play()
        const iframe = el.closest('.yws-section').querySelector('.yws-modal__inner iframe')
        const ogSrcString = iframe.dataset.ogSrc
        iframe.setAttribute('src', ogSrcString)
      }
      el.closest('.yws-section').querySelector('.yws-modal__main-area').classList.remove('active')
    })
  })
